  ...
  void AliRsnOutTaskResult::FillSparse(AliRsnOutTask *task,THnSparse *s, Int_t *sparseBin, Int_t index) {
    ...
    for (Int_t i = 0; i < task->GetListOfTasks()->GetEntries();i++) {
      ...
      if (tBin) {
        ...
        Printf("%s sparseBin[%d]=%d x=%f",t->GetName(),index,sparseBin[index],x);        
...
void AliRsnOutTaskResult::FolderFromSparse(AliRsnOutTask *task,THnSparse *s, Int_t index, TFolder *folder,Int_t indexBinFixed) {
      AliRsnOutValue *v;
