...
AliRsnOutTaskFit::AliRsnOutTaskFit(const char *name, const char *title) : AliRsnOutTask(name,title),fInput(0),fFitProbTestMin(1e-4), fFitProbTestMax(0.6),fFitResult(0),fResult(0) {
...
void AliRsnOutTaskFit::Fit(Int_t fitId, Double_t fitMin, Double_t fitMax) {
	...
	if (fResult) {
		...
		Double_t p0p = fResult->Integral(fResult->FindBin(fitMin), fResult->FindBin(fitMax))
...
//	TF1 *sigBgFnc = (TF1*) fResult->GetListOfFunctions()->At(0);
	...
	// TODO Verify it
	Double_t bgErr = bgFnc->IntegralError(fInput->GetMin(),fInput->GetMax(),fFitResult->GetParams(), fFitResult->GetCovarianceMatrix().GetMatrixArray())/histWidth;
...
void AliRsnOutTaskFit::GetYieldFitFunction(Double_t& val, Double_t& err) {
	...
	err = sigBgFnc->IntegralError(fInput->GetMin(),fInput->GetMax(),fFitResult->GetParams(), fFitResult->GetCovarianceMatrix().GetMatrixArray())/histWidth;
...
Double_t AliRsnOutTaskFit::GetChi2() {
	return fFitResult->Chi2();
}

Double_t AliRsnOutTaskFit::GetNdf() {
...
Double_t AliRsnOutTaskFit::GetProb() {
